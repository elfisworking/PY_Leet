...
""
1. 固定位置
2. 随机基点  random.randint(left, right) [left, right)
3. 三数取中 具体思想：对待排序序列中low、mid、high三个位置上数据进行排序，取他们中间的那个数据作为枢轴，并用0下标元素存储枢轴。
4. 
""
...
public:
    ...
    inline int partion(vector<int>& a, int l, int r) {
        int x = a[r], i = l - 1;
        for (int j = l; j < r; j++) {
            if(a[j] <= x) {
                swap(a[++i], a[j]);
